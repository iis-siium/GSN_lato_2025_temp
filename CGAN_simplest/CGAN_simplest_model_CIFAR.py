...
IMAGE_SAVE_PATH = "./CGAN_CIFAR10/generated_images"  # Directory to save images
SAVE_FREQUENCY = 5  # Save images every N epochs
...
class Generator(nn.Module):
    def __init__(self, latent_dim=LATENT_DIM, num_classes=NUM_CLASSES):
        ...
        # Condition could be also a simple one-hot encoded vector, but this is more general
        # Embedding layer for class condition (num_classes -> latent_dim)
        # TODO: play with the size of the embedding
        self.label_embedding = nn.Embedding(num_classes, num_classes)
        
        # Input processing
        self.input_layer = nn.Linear(latent_dim + num_classes, 4 * 4 * 512)
        
        # Convolutional layers
        self.conv_layers = nn.Sequential(
            # 4x4x512 -> 8x8x256
            nn.ConvTranspose2d(512, 256, kernel_size=4, stride=2, padding=1,
                               bias=False), # bias=False because of batch norm
                                            # https://stats.stackexchange.com/questions/482305/batch-normalization-and-the-need-for-bias-in-neural-networks
            nn.BatchNorm2d(256),
            nn.ReLU(True),
            
            # could we use something better than conv2dtranspose?
            # traditional artefacting - https://distill.pub/2016/deconv-checkerboard/
            # maybe Upsample followed by Conv2D? 
            # nn.Upsample(scale_factor=2, mode='nearest'),
            # nn.Conv2d(256, 128, kernel_size=3, stride=1, padding=1, bias=False),
            # another layer that would be an alternative is the pixel shuffle layer
            # https://stackoverflow.com/questions/72069125/is-convolution-pixelshuffle-the-same-as-subpixel-convolution
            # https://arxiv.org/abs/1609.05158
            # https://pytorch.org/docs/stable/generated/torch.nn.PixelShuffle.html
            # TODO: try to use Upsample or pixel shuffle instead of conv2dtranspose
            ...
            # 16x16x128 -> 32x32x3 (final output)
...
class Discriminator(nn.Module):
    def __init__(self, num_classes=NUM_CLASSES):
        ...
        # Convolutional layers
        self.conv_layers = nn.Sequential(
            # 32x32x3 -> 16x16x128 (first layer now takes 32x32 images)
...
class CGAN(pl.LightningModule):
    ...
    # Each module gets its own optimizer
    # TODO: could we use the same optimizer for both?
...
def main():
    ...
    checkpoint_callback = ModelCheckpoint(
        monitor='g_loss',
        dirpath='./CGAN_CIFAR10/checkpoints',
    ...
    # Save model
    trainer.save_checkpoint("cgan_final.ckpt")
